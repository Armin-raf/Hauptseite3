...
const HighlightCard: FC<ComponentProps> = ({ title, description, icon, link }) => {
  const content = (
  ...
  return (
    <div className="group cursor-pointer transform transition-all duration-500 hover:scale-105 hover:-rotate-1 w-full max-w-[350px] mx-auto h-full">
      {link ? (
        <a href={link} className="block h-full" target={link.startsWith('http') ? "_blank" : "_self"} rel="noopener noreferrer">
          {content}
        </a>
      ) : (
        content
